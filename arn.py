...
def check_arn_health(arn, region, account_id):
    ...
    try:
        if service == 'lambda':
            ...
            response = client.describe_recovery_point(BackupVaultName=resource.split(':')[1], RecoveryPointArn=recovery_point_arn)
...
def main(yaml_file):
    ...
    for component in components:
        ...
        for tf_module in tf_modules:
            ...
            for arn in arns_list:
                ...
                # "Ping" the resource by checking health
                healthy, message = check_arn_health(arn_val, region, account_id)
                if not healthy:
                    all_healthy = False
                messages.append(f"{arn_val} ({component_name}, {component_type}): {message}")
                ...
                arn_results.append(arn_result)
                logging.info(f"Checked {arn_val} ({component_name}, {component_type}): {'healthy' if healthy else 'unhealthy'} - {message}")
    ...
    output = {
        ...
        "data": {
            ...
            "arnResults": arn_results  # New field with list of ARNs and their details
        }
    }
